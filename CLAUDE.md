...
- **Every settings page/tab gets a UNIQUE icon.** No two settings tabs (core or addon) may share an icon - falling back to the generic default gear for an addon tab is a bug, not a placeholder. When PressPrimer Quiz or Assignment has a page with the same function, use THEIR icon (e.g. `license` is `KeyOutlined` in both); check their `SettingsPage.jsx` icon maps before choosing. New addon tab ids must be added to the free `ADDON_ICONS` map (`src/settings/components/SettingsPage.jsx`) in the same change that registers the tab.
