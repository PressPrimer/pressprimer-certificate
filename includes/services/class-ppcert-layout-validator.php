...
class PressPrimer_Certificate_Layout_Validator {
	 ...
	 *
	 * Provisional single-font set until the bundled font manifest lands
	 * (Prompt 2.2), which becomes the canonical source for this list.
	 *
	 * @since 1.0.0
	 *
	...
	public static function get_registered_font_slugs() {
		 ...
		 * bundled font manifest (Prompt 2.2) populates the default set;
		 * Educator 2.0 adds custom fonts through this same filter.
		 ...
		 */
		$fonts = apply_filters( 'ppcert_designer_fonts', [ self::DEFAULT_FONT => [] ] );

		return array_map( 'strval', array_keys( (array) $fonts ) );
	}
